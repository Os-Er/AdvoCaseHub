...
interface DataUploaderProps {
  action: (prev: ImportSonuc | null, fd: FormData) => Promise<ImportSonuc>;
  baslik: string;
  aciklama: string;
  sablonIndir: () => void;
  sablonAciklama: string;
  sutunlar: SutunTanim[];
  geriHref: string;
  listHref: string;
  listButonMetni: string;
}
...
export function DataUploader({
  ...
  const [sonuc, formAction, pending] = useActionState<ImportSonuc | null, FormData>(action, null);
  ...
  const handleDragOver  = useCallback((e: React.DragEvent) => { e.preventDefault(); setSurukleAktif(true); }, []);
  const handleDragLeave = useCallback((e: React.DragEvent) => { e.preventDefault(); setSurukleAktif(false); }, []);
  const handleDrop      = useCallback((e: React.DragEvent) => {
    e.preventDefault(); setSurukleAktif(false);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* Başlık */}
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "#1B2A4A" }}>{baslik}</h1>
        <p className="text-slate-500 mt-1 text-sm">{aciklama}</p>
      </div>

      {/* Adımlar */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          { num: "1", title: "Şablonu İndir",  desc: "CSV formatını ve örnek satırı al" },
          ...
          <div key={s.num} className="bg-white rounded-xl border border-slate-200 p-4 flex items-start gap-3">
            <span className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 text-white"
              style={{ backgroundColor: "#1B2A4A" }}>{s.num}</span>
            <div>
      ...
      <div className="bg-white rounded-xl border-2 p-5 space-y-4" style={{ borderColor: "#D4AF37" }}>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h3 className="font-semibold text-slate-800">CSV Şablonu — {sutunlar.length} Sütun</h3>
            <p className="text-sm text-slate-500 mt-0.5">{sablonAciklama}</p>
          </div>
          <button
            onClick={sablonIndir}
        ...
        <button type="button" onClick={() => setSutunlarAcik((v) => !v)}
          className="text-xs font-medium flex items-center gap-1 transition-colors"
          style={{ color: "#B8962A" }}>
          <span>{sutunlarAcik ? "▾" : "▸"}</span>
          {sutunlarAcik ? "Sütun kılavuzunu gizle" : `Sütun kılavuzunu göster (${sutunlar.length} alan)`}
        </button>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-slate-50 border-b">
                  ...
                  <th className="text-left px-3 py-2 font-semibold text-slate-600">Sütun</th>
                ...
                {sutunlar.map((s, i) => (
                  <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                    ...
                    <td className="px-3 py-1.5 font-medium text-slate-800 whitespace-nowrap">
                      {s.ad}
                      {s.zorunlu && <span className="text-red-500 ml-0.5">*</span>}
                    </td>
        ...
        <form action={formAction} className="space-y-4">
          {!secilenDosya ? (
            ...
            <div className="flex items-center gap-4 p-4 rounded-xl border-2"
              ...
              <button type="button" onClick={dosyayiKaldir}
          ...
          <input ref={inputRef} type="file" name="csv_dosya" accept=".csv,text/csv"
            className="hidden" onChange={handleInputChange} />

          <div className="flex items-center justify-between gap-3">
            <p className="text-xs text-slate-400">Hatalı satırlar atlanır, geçerli satırlar eklenir.</p>
            <Button type="submit" disabled={pending || !secilenDosya}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="rounded-xl border p-4 text-center"
              ...
              <p className="text-xs font-medium uppercase tracking-wide mb-1"
                style={{ color: sonuc.basarili > 0 ? "#065F46" : "#64748B" }}>Başarıyla Eklendi</p>
              <p className="text-2xl font-bold"
                style={{ color: sonuc.basarili > 0 ? "#059669" : "#CBD5E1" }}>{sonuc.basarili}</p>
            </div>
            <div className="rounded-xl border p-4 text-center"
              style={{
              ...
              <p className="text-xs font-medium uppercase tracking-wide mb-1"
                style={{ color: sonuc.hatalar.length > 0 ? "#991B1B" : "#64748B" }}>Atlanan Satır</p>
              <p className="text-2xl font-bold"
                style={{ color: sonuc.hatalar.length > 0 ? "#DC2626" : "#CBD5E1" }}>{sonuc.hatalar.length}</p>
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2">
              ...
              {sonuc.basarili > 0 && (
                <p className="text-xs text-slate-500 flex items-center gap-1">
                  ...
                  {sonuc.basarili} kayıt başarıyla eklendi.
                </p>
              )}
            </div>
          )}
              ...
              <div className="divide-y divide-slate-100 max-h-72 overflow-y-auto">
                {sonuc.hatalar.map((h, i) => (
                  <div key={i} className="px-4 py-2.5 flex items-start gap-3">
                    <span className={`inline-flex items-center justify-center px-2 h-5 rounded text-xs font-mono font-semibold flex-shrink-0 mt-0.5 whitespace-nowrap ${h.satir > 0 ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-600"}`}>
                      {h.satir > 0 ? `Satır ${h.satir}` : "Genel"}
                    </span>
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            {sonuc.basarili > 0 ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-emerald-700">{sonuc.basarili} kayıt eklendi</span>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                <XCircle className="w-5 h-5 text-slate-400" />
                <span className="text-sm text-slate-500">Hiç kayıt eklenemedi</span>
              </div>
            )}
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => { dosyayiKaldir(); window.location.reload(); }}>
                Yeni Aktarım
              </Button>
              {sonuc.basarili > 0 && (
                <Link href={listHref}>
