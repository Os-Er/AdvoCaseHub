...
import {
  ...
  deleteSureliIs,
} from "@/lib/actions/sureli-isler";

export function TamamlaButonu({ id }: { id: string }) {
  const [pending, startTransition] = useTransition();
  function handleClick() {
    if (!confirm("Bu işi tamamlandı olarak işaretlemek istiyor musunuz?")) return;
    startTransition(async () => {
      const res = await tamamlaSureliIs(id);
      if ("error" in res) toast.error(res.error);
      else toast.success("İş tamamlandı olarak işaretlendi.");
    });
  }
  return (
    <button onClick={handleClick} disabled={pending}
      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md border border-green-200 text-green-700 bg-green-50 hover:bg-green-100 disabled:opacity-50 transition-colors">
...
export function ArsivleButonu({ id }: { id: string }) {
  const [pending, startTransition] = useTransition();
  function handleClick() {
    if (!confirm("Bu işi arşivlemek istediğinizden emin misiniz?")) return;
    startTransition(async () => {
      const res = await arsivleSureliIs(id);
      if ("error" in res) toast.error(res.error);
      else toast.success("İş arşivlendi.");
    });
  }
  return (
    <button onClick={handleClick} disabled={pending}
...
export function ArsivdenCikarButonu({ id }: { id: string }) {
  ...
  function handleClick() {
    startTransition(async () => {
      const res = await arsivdenCikarSureliIs(id);
      if ("error" in res) toast.error(res.error);
      else toast.success("İş aktif listeye döndürüldü.");
    });
  }
  return (
    <button onClick={handleClick} disabled={pending}
...
export function KaliciSilButonu({ id }: { id: string }) {
  ...
  function handleConfirm() {
    startTransition(async () => {
      const res = await deleteSureliIs(id);
      if ("error" in res) toast.error(res.error);
      else toast.success("İş kalıcı olarak silindi.");
      setOpen(false);
    });
  }
  if (open) {
