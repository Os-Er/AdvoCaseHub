...
import {
  Plus, FolderOpen, Upload, ArrowUpDown,
  ArrowUp, ArrowDown, Calendar, Archive,
} from "lucide-react";
...
import { ArsivleButonu } from "@/components/dosyalar/arsiv-butonu";
import type { MaliDurumOzet, MakbuzOzetInput } from "@/components/dosyalar/mali-durum-badge";
import type { Kategori, DosyaDurum, DosyaTip } from "@/lib/types/database";
...
const TIP_KONFIG: Record<DosyaTip, {
  ...
  HUKUK: {
    baslik: "Hukuk Davaları",
    aciklama: "Hukuk mahkemelerindeki aktif dosyalar",
    merkezLabel: "Mahkeme",
    dosyaNoLabel: "Esas No",
    tarihLabel: "Duruşma Tarihi",
    arsivHref: "/dosyalar/hukuk/arsiv",
    yeniHref: "/dosyalar/yeni?tip=HUKUK",
    listHref: "/dosyalar/hukuk",
  },
  CEZA: {
    baslik: "Ceza Davaları",
    aciklama: "Ceza mahkemelerindeki ve savcılıktaki aktif dosyalar",
    merkezLabel: "Savcılık / Mahkeme",
    dosyaNoLabel: "Soruşturma / Kovuşturma No",
    tarihLabel: "Duruşma Tarihi",
    arsivHref: "/dosyalar/ceza/arsiv",
    yeniHref: "/dosyalar/yeni?tip=CEZA",
    listHref: "/dosyalar/ceza",
  },
  ICRA: {
    baslik: "İcra Takipleri",
    aciklama: "İcra dairelerindeki aktif takip dosyaları",
    merkezLabel: "İcra Dairesi",
    dosyaNoLabel: "İcra Dosya No",
    tarihLabel: "Takip Tarihi",
    arsivHref: "/dosyalar/icra/arsiv",
    yeniHref: "/dosyalar/yeni?tip=ICRA",
    listHref: "/dosyalar/icra",
  },
};
...
export interface DosyaListesiProps {
  tip: DosyaTip;
...
export async function DosyaListesi({ tip, searchParams }: DosyaListesiProps) {
  ...
  function maliSiralamaHref() {
    ...
    return `${konfig.listHref}?${new URLSearchParams({ ...sp, siralama: sonraki, sayfa: "1" })}`;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">{count ?? 0} aktif dosya</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <Link
            href={konfig.arsivHref}
          ...
          <ExportButton
            ...
            params={{ q, durum, kategori, yil, tip }}
            label="CSV İndir"
          />
          <Link
            href={`/dosyalar/import?tip=${tip}`}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
        {yaklasanFiltre && (
          <div className="flex items-center justify-between px-3 py-2 rounded-lg border"
            ...
            <div className="flex items-center gap-2 text-sm font-medium text-blue-700">
              ...
              Yaklaşan Duruşmalar — bugün ve sonrası, tarihe göre sıralı
            </div>
            <Link
              href={`${konfig.listHref}?${new URLSearchParams({ q, durum, kategori, yil })}`}
        ...
        <form method="GET" className="flex items-center gap-2 flex-wrap">
          ...
          {yil && (
            <Link href={`${konfig.listHref}?${new URLSearchParams({ q, durum, kategori })}`}>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-slate-50">
                    <th className="text-left px-4 py-3 font-medium text-slate-600">Klasör / {konfig.dosyaNoLabel}</th>
                    ...
                    <th className="text-left px-4 py-3 font-medium text-slate-600">
                      ...
                      >
                        Mali Durum
                        <SiralamaIkonu aktif={siralama === "mali_asc" || siralama === "mali_desc"} siralama={siralama} />
                      </Link>
                    </th>
            ...
            <div className="px-4 py-3 border-t bg-slate-50 flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                {siralama && (
                  <span className="inline-flex items-center gap-1 text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
                    ...
                    <Link
                      href={`${konfig.listHref}?${new URLSearchParams({ q, durum, kategori, yil, sayfa: "1" })}`}
              ...
              {toplamSayfa > 1 && (
                <div className="flex gap-2">
                  {sayfa > 1 && (
                    <Link href={`${konfig.listHref}?${new URLSearchParams({ ...sp, sayfa: String(sayfa - 1) })}`}>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-slate-500 font-medium">
              {q || durum || kategori || yil
                ...
                : `Henüz ${konfig.baslik.toLowerCase()} eklenmemiş.`}
            </p>
            {!q && !durum && !kategori && !yil && (
