...
export default async function MakbuzYeniPage() {
  const kaynaklar = await fetchKaynakSecenekleri();
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <Link href="/finans/makbuzlar">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1 text-sm">Müvekkile kesilen makbuz bilgilerini girin.</p>
      </div>
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <FinansFormu action={createFinans} cancelHref="/finans/makbuzlar" kaynaklar={kaynaklar} defaultTip="MAKBUZ" />
      </div>
    </div>
  );
}
